...
const RecipeForm: React.FC<RecipeFormProps> = ({ onClose, recipeToEdit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (recipeToEdit) {
      updateRecipe({ ...recipeData, id: recipeToEdit.id, createdAt: recipeToEdit.createdAt });
    } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <Input label="Tytuł przepisu" value={title} onChange={(e) => setTitle(e.target.value)} error={errors.title} required />
      ...
      <div>
        ...
        {ingredients.map((ingredient, index) => (
          <div key={ingredient.id} className="flex items-center space-x-2 mb-2">
            <Input containerClassName="mb-0 flex-1" placeholder="Nazwa" value={ingredient.name} onChange={(e) => handleIngredientChange(index, 'name', e.target.value)} />
            <Input containerClassName="mb-0 w-24" placeholder="Ilość" value={ingredient.quantity} onChange={(e) => handleIngredientChange(index, 'quantity', e.target.value)} />
            <Input containerClassName="mb-0 w-28" placeholder="Jednostka" value={ingredient.unit} onChange={(e) => handleIngredientChange(index, 'unit', e.target.value)} />
            <Button type="button" variant="danger" size="sm" onClick={() => removeIngredientField(ingredient.id)} disabled={ingredients.length === 1}>
              <TrashIcon />
            </Button>
          </div>
        ))}
        {errors.ingredients && <p className="text-xs text-red-600">{errors.ingredients}</p>}
        <Button type="button" variant="secondary" size="sm" onClick={addIngredientField} leftIcon={<PlusIcon />}>
          Dodaj składnik
        </Button>
      </div>

      <Textarea label="Instrukcje przygotowania" value={instructions} onChange={(e) => setInstructions(e.target.value)} error={errors.instructions} required />
      <Input label="Czas przygotowania (np. 30 minut)" value={prepTime} onChange={(e) => setPrepTime(e.target.value)} error={errors.prepTime} required />
      <Select label="Kategoria" options={RECIPE_CATEGORIES_OPTIONS} value={category} onChange={(e) => setCategory(e.target.value as RecipeCategory)} required />
      ...
      <div>
        ...
        <div className="flex items-center space-x-2 mb-2">
          ...
          />
          <Button type="button" variant="secondary" size="sm" onClick={handleTagAdd}>Dodaj tag</Button>
        </div>
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {tags.map(tag => (
              <span key={tag} className="bg-sky-100 text-sky-700 px-2 py-1 rounded-full text-sm flex items-center">
                {tag}
                <button type="button" onClick={() => handleTagRemove(tag)} className="ml-1 text-sky-500 hover:text-sky-700">
                  &times;
                </button>
              </span>
            ))}
      ...
      <div className="flex justify-end space-x-3 pt-4">
        ...
        <Button type="submit" variant="primary">{recipeToEdit ? 'Zapisz zmiany' : 'Dodaj przepis'}</Button>
      </div>
    </form>
  );
};
